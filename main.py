...
# 获取环境变量，如果环境变量没有设置，则使用默认值

# 最大线程数，默认值为 10
max_thread = int(os.environ.get("MAX_THREAD") or 10)

# 查询超时时间，默认值为 2 秒
timeout = int(os.environ.get("TIMEOUT") or 2)

# 查询之间的休眠时间，默认值为 1 秒
sleep_time = int(os.environ.get("SLEEP_TIME") or 1)

# 域名后缀，默认值为 'cn'
domain_name_suffix = os.environ.get("DOMAINM_SUFFIX") or 'cn'

# 域名最大长度，默认值为 7
domain_name_length = int(os.environ.get("DOMAIN_NAME_LENGTH") or 7)

# 域名字典文件路径，默认值为 './dictMap/26pl1+2+3+4.txt'
domain_dictionary =  os.environ.get("DOMAIN_DICTIONARY") or './dictMap/26pl1+2+3+4.txt'
...
def get_reginfomation(domain_name, domain_name_suffix_infomation):
    ...
    if not infomation:
        with open(f'logs/failure.txt','a') as f:
    ...
    if infomation.find(reg) >= 0:
        ...
        print(f'域名{domain_name}.{domain_name_suffix_infomation[0]} 未注册，已保存在本程序同目录下的"logs/success.txt"文件中')
    else:
...
def specify_suffix_and_dictionary():
    # domain_name_suffix = os.environ.get("DOMAINMSUFFIX") or 'com' # input("输入域名后缀(如:com,cn)____:")
    # domain_dictionary = './dic字典库/26pl1+2+3+4.txt' # input("输入字典名(如:demo.txt)____:")
    # domain_name_length = 6 # int(input("输入过滤长度(如:7)____:"))
